...
func TestDrainChannelN(t *testing.T) {
	...
	result := sharkfunc.DrainChannelN(context.Background(), ch, 5)
	if len(result) != 5 {
...
func TestDrainChannelNWithCtxCancel(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	result := sharkfunc.DrainChannelN(ctx, ch, 10)
	if len(result) != 0 {
...
func TestDrainChannelNWithClosedChannel(t *testing.T) {
	...
	ch <- 2
	close(ch)
	result := sharkfunc.DrainChannelN(context.Background(), ch, 10)
	if len(result) != 2 {
...
func TestDrainChannelNWithNilChannel(t *testing.T) {
	result := sharkfunc.DrainChannelN[int](context.Background(), nil, 10)
	if len(result) != 0 {
